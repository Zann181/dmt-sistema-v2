...
- Fixed a massive NextAuth JWT stale token security bug in `auth.ts`. Previously, a user's permissions were permanently locked into their JWT cookie at the moment of sign-in. If an administrator downgraded a user's permissions in the DB, the user would retain administrative access until they manually logged out. The `session` callback now performs a dynamic database lookup on every session request, guaranteeing that role downgrades and event assignments take effect immediately.
- Entrada UI deduplicada. Submenús `add-user`/`edit-event` eliminados. Rol `accessAttendees` usa módulos globales `Eventos`/`Usuarios` con acceso restringido. API `usuarios` restringe no-admins a sus propios eventos. UI oculta toggles Superusuario/Admin Global para prevenir escalado de privilegios.
