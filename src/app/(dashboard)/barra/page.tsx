    ...
    <div className="flex h-[calc(100vh-8rem)] gap-6 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            <div className="text-center text-emerald-400 py-12 text-sm">Cargando menú del evento...</div>
          ) : error ? (
              ...
              {products?.map(p => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    {p.description && <span className="text-[10px] text-emerald-300 truncate mb-1">{p.description}</span>}
      ...
      <div className="w-96 flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden shrink-0">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-950/20">
          ...
          <button onClick={clear} className="text-emerald-300 hover:text-red-500 transition-colors">
            <Trash2 size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-emerald-300 space-y-2">
              ...
              <div key={item.eventProductId} className="flex items-center justify-between p-3 bg-zinc-50/50 dark:bg-zinc-950/40 rounded-xl border border-zinc-100 dark:border-zinc-800/60 shadow-sm">
                ...
                <div className="flex items-center gap-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-1.5 shadow-sm">
                  <button 
                    ...
                    className="p-1 text-emerald-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors"
                  >
                    <Minus size={12} />
                  </button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b pb-3 border-zinc-200 dark:border-zinc-800">
              ...
              <button onClick={() => setCheckoutType(null)} className="text-emerald-300 hover:text-zinc-600 dark:hover:text-zinc-200">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleConfirmCheckout} className="space-y-4">
              <div className="bg-zinc-50 dark:bg-zinc-900 p-4 rounded-lg flex items-center justify-between">
                <span className="font-semibold text-emerald-400 text-sm">Total del Pedido:</span>
                <span className="text-xl font-black">${total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>

              {checkoutType === "CASH" && (
                <div>
                  <label className="text-xs font-semibold text-emerald-400 block mb-1">Monto Recibido</label>
                  <input
                    type="text"
                    placeholder="0"
                    value={cashReceived}
              ...
              {checkoutType === "CARD" && (
                <div>
                  <label className="text-xs font-semibold text-emerald-400 block mb-1">Referencia del Voucher (Opcional)</label>
                  <input
                    type="text"
              ...
              {checkoutType === "MIXED" && (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-semibold text-emerald-400 block mb-1">Monto en Efectivo</label>
                      <input
                        type="text"
                        placeholder="0"
                        value={mixedCash}
                    ...
                    <div>
                      <label className="text-xs font-semibold text-emerald-400 block mb-1">Monto en Tarjeta</label>
                      <input
                        type="text"
                        placeholder="0"
                        value={mixedCard}
                  ...
                  <div className="flex items-center justify-between text-xs font-semibold border-t pt-3 border-zinc-100 dark:border-zinc-800">
                    <span className="text-emerald-400">Restante por pagar:</span>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-in zoom-in-95 duration-150">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-5 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-300 font-mono mt-1">Ref: {successDetails.saleGroupId}</p>
            </div>

            <div className="border-t border-b border-zinc-100 dark:border-zinc-800/80 py-4 text-left space-y-2">
              <span className="text-xs font-semibold text-emerald-300 uppercase tracking-wider block mb-2">Detalles del Pedido</span>
              {successDetails.cart.map((item: any) => (
                <div key={item.eventProductId} className="flex justify-between text-sm">
                  <span className="text-zinc-600 dark:text-emerald-300">{item.name} <span className="text-emerald-300">x{item.quantity}</span></span>
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 p-3 rounded-xl text-left space-y-1.5 text-xs text-emerald-400">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b pb-3 border-zinc-200 dark:border-zinc-800">
              ...
              <button onClick={() => setShowCashModal(false)} className="text-emerald-300 hover:text-zinc-600 dark:hover:text-zinc-200">
                <X size={18} />
              </button>
            </div>
