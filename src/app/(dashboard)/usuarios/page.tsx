    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-lime-400">Administra accesos globales, asignaciones a sucursales y roles.</p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-zinc-800">
              <tr>
                <th className="px-6 py-3 font-medium text-lime-400">Nombre / Usuario</th>
                <th className="px-6 py-3 font-medium text-lime-400">Roles Globales</th>
                <th className="px-6 py-3 font-medium text-lime-400">Sucursales</th>
                <th className="px-6 py-3 font-medium text-lime-400">Eventos</th>
                <th className="px-6 py-3 font-medium text-lime-400">Estado</th>
                <th className="px-6 py-3 font-medium text-lime-400 text-right">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {usersLoading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-lime-400">Cargando usuarios...</td>
                </tr>
              ) : users?.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-lime-400">No hay usuarios en el sistema.</td>
                </tr>
              ) : (
                users?.map((u) => (
                  <tr key={u.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-lime-400">@{u.username} &bull; {u.email}</p>
                    </td>
                    <td className="px-6 py-4 space-y-1">
                      ...
                      {!u.isSuperuser && !u.isGlobalAdmin && (
                        <span className="text-lime-300 text-xs">Personal Estándar</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1.5 max-w-xs">
                        {u.branchMemberships?.map((m: any, idx: number) => (
                          <span key={`${m.id}-${idx}`} className="px-2 py-0.5 rounded text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-lime-300 border border-zinc-200 dark:border-zinc-700">
                            {m.branch.name} ({m.role})
                          </span>
                        ))}
                        {(!u.branchMemberships || u.branchMemberships.length === 0) && (
                          <span className="text-lime-300 text-xs">Sin sucursales</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1.5 max-w-xs">
                        ...
                        {(!u.eventAssignments || u.eventAssignments.length === 0) && (
                          <span className="text-lime-300 text-xs">Sin eventos</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.isActive ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-lime-300"}`}>
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button 
                        ...
                        className="p-1.5 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-lime-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button 
                        onClick={() => { if(confirm("¿Eliminar usuario?")) deleteUserMutation.mutate(u.id) }}
                        className="p-1.5 border border-zinc-200 dark:border-zinc-800 text-lime-300 hover:text-red-600 hover:border-red-200 dark:hover:text-red-400 dark:hover:border-red-900 rounded transition-colors"
                      >
                        <Trash2 size={14} />
                      </button>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); createUserMutation.mutate(createForm) }} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-lime-400 block mb-1">Nombre de Usuario</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-xs font-semibold text-lime-400 block mb-1">Correo Electrónico</label>
                <input 
                  type="email" 
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-semibold text-lime-400 block mb-1">Apellido</label>
                  <input 
                    type="text" 
              ...
              <div>
                <label className="text-xs font-semibold text-lime-400 block mb-1">Contraseña</label>
                <input 
                  type="password" 
              ...
              {isSuper && (
                <div className="border-t pt-4 space-y-3">
                  <p className="text-xs font-bold text-lime-300 uppercase tracking-wider mb-2">Permisos y Roles Globales</p>
                  ...
                  <label className="flex items-center justify-between p-2 border rounded-md cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                    <div>
                      ...
                      <p className="text-xs text-lime-400">Acceso total absoluto al sistema</p>
                    </div>
                    <button 
                      type="button"
                      onClick={() => setCreateForm({ ...createForm, isSuperuser: !createForm.isSuperuser })}
                      className="text-lime-400 hover:text-indigo-600 transition-colors"
                    >
                  ...
                  <label className="flex items-center justify-between p-2 border rounded-md cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                    <div>
                      ...
                      <p className="text-xs text-lime-400">Permisos de gestión sin límite de sucursal</p>
                    </div>
                    <button 
                      type="button"
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto space-y-4">
            <div className="flex items-center justify-between border-b pb-3 border-zinc-200 dark:border-zinc-800">
              ...
              <button onClick={() => { setShowEditModal(false); setSelectedUser(null) }} className="text-lime-300 hover:text-zinc-600 dark:hover:text-zinc-200 text-sm font-semibold">✕</button>
            </div>
            ...
            <div className="flex border-b border-zinc-200 dark:border-zinc-800 mb-2">
              <button
                ...
                className={`flex-1 pb-2 text-sm font-semibold border-b-2 text-center transition-colors ${activeEditTab === "profile" ? "border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400" : "border-transparent text-lime-400 hover:text-zinc-700 dark:hover:text-zinc-300"}`}
              >
                Perfil
              </button>
              <button
                ...
                className={`flex-1 pb-2 text-sm font-semibold border-b-2 text-center transition-colors ${activeEditTab === "branches" ? "border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400" : "border-transparent text-lime-400 hover:text-zinc-700 dark:hover:text-zinc-300"}`}
              >
                Sucursales
              </button>
              <button
                ...
                className={`flex-1 pb-2 text-sm font-semibold border-b-2 text-center transition-colors ${activeEditTab === "events" ? "border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400" : "border-transparent text-lime-400 hover:text-zinc-700 dark:hover:text-zinc-300"}`}
              >
                Eventos
              </button>
            </div>
            ...
            {activeEditTab === "profile" && (
              <form onSubmit={(e) => { e.preventDefault(); editUserMutation.mutate(editForm) }} className="space-y-4 pt-2">
                ...
                <div>
                  <label className="text-xs font-semibold text-lime-400 block mb-1">Cambiar Contraseña</label>
                  <input 
                    type="password" 
                ...
                <div className="pt-2 space-y-2 border-t border-zinc-200 dark:border-zinc-800">
                  <p className="text-xs font-bold text-lime-300 uppercase tracking-wider mb-2">Estado y Roles Globales</p>

                  <label className="flex items-center justify-between p-2 border rounded-md cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                    <div>
                      <p className="text-sm font-semibold">Usuario Activo</p>
                      <p className="text-xs text-lime-400">Permite o niega el login</p>
                    </div>
                    <button 
                      type="button"
                  ...
                  {isSuper && (
                    <>
                      ...
                      <label className="flex items-center justify-between p-2 border rounded-md cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                        <div>
                          <p className="text-sm font-semibold">Administrador Global</p>
                          <p className="text-xs text-lime-400">Permisos globales de gestión</p>
                        </div>
                        <button 
                          type="button"
            ...
            {activeEditTab === "branches" && (
              <div className="space-y-4 pt-2">
                <h4 className="font-semibold text-sm flex items-center gap-2">
                  <Store size={18} className="text-lime-400" /> Asignar a Sucursales
                </h4>
                ...
                <div className="bg-zinc-50 dark:bg-zinc-950 p-4 border border-zinc-200 dark:border-zinc-800 rounded-lg space-y-3">
                  ...
                  <div>
                    <label className="text-xs font-semibold text-lime-400 block mb-1">Rol en Sucursal</label>
                    <select 
                ...
                <div className="space-y-2">
                  <p className="text-xs font-bold text-lime-300 uppercase tracking-wider">Sucursales Activas</p>
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {selectedUser.branchMemberships?.map((m: any, idx: number) => (
                      <div key={`${m.id}-${idx}`} className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-lime-400">{m.role}</p>
                        </div>
                        <button 
                          onClick={() => removeMembershipMutation.mutate(m.id)}
                          className="p-1 text-lime-300 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                    ...
                    {(!selectedUser.branchMemberships || selectedUser.branchMemberships.length === 0) && (
                      <p className="text-sm text-lime-400 text-center py-4">No tiene asignaciones activas</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {activeEditTab === "events" && (
              <div className="space-y-4 pt-2">
                <h4 className="font-semibold text-sm flex items-center gap-2">
                  <Calendar size={18} className="text-lime-400" /> Asignar a Eventos
                </h4>
                ...
                <div className="bg-zinc-50 dark:bg-zinc-950 p-4 border border-zinc-200 dark:border-zinc-800 rounded-lg space-y-3">
                  ...
                  <div>
                    <label className="text-xs font-semibold text-lime-400 block mb-1">Evento</label>
                    <select 
                  ...
                  <div>
                    <label className="text-xs font-semibold text-lime-400 block mb-1">Rol en Evento</label>
                    <select 
                ...
                <div className="space-y-2">
                  <p className="text-xs font-bold text-lime-300 uppercase tracking-wider">Eventos Activos</p>
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {selectedUser.eventAssignments?.map((a: any, idx: number) => (
                      <div key={`${a.id}-${idx}`} className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-lime-400">{a.role}</p>
                        </div>
                        <button 
                    ...
                    {(!selectedUser.eventAssignments || selectedUser.eventAssignments.length === 0) && (
                      <p className="text-sm text-lime-400 text-center py-4">No tiene asignaciones activas de evento</p>
                    )}
                  </div>
                </div>
              </div>
